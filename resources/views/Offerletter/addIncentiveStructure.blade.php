...
.panel-body {
	...
	<br>
	<form action="{{ url('addIncentiveStructurePost') }}" id="incentiveFrm" method="post" enctype="multipart/form-data">
    ...
    <div class="panel-heading">
		<div class="col-lg-4"> Manage Incentive Breakup </div>
		 <div class="col-lg-8 filter-button text-right">
										 ...
										 <a class="btn btn-light" href="{{url('manageIncentive')}}" role="button"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back</a>
    ...
    <div class="panel-body">
        ...
        <div class="row">
          ...
          <div class="col-lg-12">
              ...
              <select class="selectpicker form-control" data-live-search="true" id="breakup_id" name="incentiveBreakup[breakup_id]" onchange="resetMe();">
			...
			<div class="form-group col-md-4">
					  ...
					  <select class="selectpicker form-control" data-live-search="true" id="incentive_type" name="incentiveBreakup[incentive_type]" onchange="resetMe();">
        ...
        <div class="row">
	  	...
	  	<div id="breackupContent">
				<div class="col-lg-12">
					...
					<div class="form-group col-md-4 aed-cont">
					  <label>Incentive<em>*</em></label>
						<div class="input-group-prepend"> <span  class="incentiveClassAmt input-group-text">AED</span></div><input type="text" placeholder="Enter Incentive" class="IncentiveClass" id="values_1" name="incentiveBreakup[values][]" autocomplete="off">
					</div>
...
function createIncentiveStructure()
{
	...
	else if(jQuery("#incentive_type").val() == '')
	{
		jQuery("#errorContent").html('select Incentive Type to proceed.');
		jQuery('#errorPopup').modal('show');
	}
	else
	{
		...
		jQuery.ajax({
						...
						success: function(response){
							...
							if(response == 'Allow')
							{
								...
								if(jQuery("#incentive_type").val() == 'number_count')
								{
									jQuery(".incentiveClassAmt").html("AED");
									
								}
								else if(jQuery("#incentive_type").val() == 'Amount')
								{
									jQuery(".incentiveClassAmt").html("%");
							...
							{
								jQuery("#errorContent").html('Incentive Break already exist for selected Caption. Please first delete exist one then try again.');
								jQuery('#errorPopup').modal('show');
							}
						}
					});
...
function addMoreBreakup()
{
	...
	var html = '<div class="col-lg-12" id="row_'+newidValCountSet+'"><div class="form-group col-md-3"><label>From<em>*</em></label> <span  class="frmClassAmt"></span><input type="text" placeholder="Enter Range" class="fromInc" id="from_inc_'+newidValCountSet+'" name="incentiveBreakup[from_inc][]" autocomplete="off"></div><div class="form-group col-md-3"> <label>To<em>*</em></label><span  class="toClassAmt"></span><input type="text" placeholder="Enter Range" class="toInc" id="to_inc_'+newidValCountSet+'" name="incentiveBreakup[to_inc][]" autocomplete="off"></div><div class="form-group col-md-3 aed-cont"> <label>Incentive<em>*</em></label><div class="input-group-prepend"> <span  class="incentiveClassAmt input-group-text">AED</span></div><input type="text" placeholder="Enter Incentive" class="IncentiveClass" id="values_'+newidValCountSet+'" name="incentiveBreakup[values][]" autocomplete="off"></div><div class="form-group col-md-3" style="padding-right: 0;    margin-top: 22px;" ><a class="btn btn-danger" href="javascript:void(0);" role="button" style="    width: 100%;padding: 8px 12px;margin-top: 3px;font-weight: bold;float: right;margin-right: 15px;" onclick="removerow('+newidValCountSet+','+newidValCountSet+');"><i class="fa fa-times" aria-hidden="true" style="margin-right: 5px;"></i>Remove</a></div></div>';
...
function saveBreakup()
{
	...
	var countErrorFlag = 0;
	 ...
	 for(j=0;j<idValStrArray.length;j++)
		{
			var valueInc = jQuery.isNumeric(jQuery("#values_"+idValStrArray[j]).val());
			...
			else if(jQuery("#to_inc_"+idValStrArray[j]).val() == '')
			{
				countErrorFlag = 1;
				jQuery("#errorContent").html('you can not Leave blank.');
			...
			{
				countErrorFlag = 1;
				jQuery("#errorContent").html('Incentive should be numeric.');
				jQuery("#values_"+idValStrArray[j]).css("border","2px solid red");
				jQuery('#errorPopup').modal('show');
			}
			else
...
jQuery("#breackupContent").html('<div class="col-lg-12"><div class="form-group col-md-4"><label>From<em>*</em></label> <input type="text" placeholder="Enter Range" class="fromInc" id="from_inc_1" name="incentiveBreakup[from_inc][]" autocomplete="off"></div><div class="form-group col-md-4"><label>To<em>*</em></label><input type="text" placeholder="Enter Range" class="toInc" id="to_inc_1" name="incentiveBreakup[to_inc][]" autocomplete="off"></div><div class="form-group col-md-4 aed-cont"> <label>Incentive<em>*</em></label><div class="input-group-prepend"> <span  class="incentiveClassAmt input-group-text">AED</span></div><input type="text" placeholder="Enter Incentive" class="IncentiveClass" id="values_1" name="incentiveBreakup[values][]" autocomplete="off"></div></div>');

jQuery("#labelBreakup").val(1);
jQuery("#idVal").val(1);
jQuery("#idValCount").val(1);
}
