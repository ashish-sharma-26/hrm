...
class OfferController extends Controller
{
	...
	public function createPDF(Request $request)
	{
		...
		*/
		...
		$this->fpdf->Text($x+9.5,$y,$empName);
		$y = $y+7;
		$this->fpdf->SetFont('Arial','B',8);
		...
		$this->fpdf->SetFont('Arial','B',8);
		$this->fpdf->Text($x+11,$y,'Bishal Chakraborty ("Employee") with Smart Union Commercial Brokerage LLC.');
		$y = $y+7;
		$this->fpdf->SetFont('Arial','',8);
		...
		$this->fpdf->SetFont('Arial','B',8);
		$this->fpdf->Text($x+88,$y,$designationName.' ('.$productName.'- '.$departmentName.')');
		$this->fpdf->SetFont('Arial','',8);
		$this->fpdf->Text($x+160,$y,'and this appointment');
		$y = $y+4;
		$this->fpdf->SetFont('Arial','',8);
		$this->fpdf->Text($x+13,$y,'is subject  to necessary completion of the Labour');
		 
		
		$this->fpdf->Text($x+75,$y,'and Immigration formalitites of the United Arab Emirates. The general terms and conditi-');
		$y = $y+4;
		$this->fpdf->Text($x+13,$y,'ons of the appointment are as follows.');
		...
		$y = $y+4;
		$this->fpdf->Text($x+20,$y,'T.Rajan');
		$y = $y+4;
		$this->fpdf->Text($x+17,$y,'HR Manager');
		$y = $y+17;
		...
		$this->fpdf->SetFont('Arial','',8);
		$this->fpdf->Text($x+32,$y,'understand and accept the offer of employment and agree to the terms and conditions as detailed here in.');
		$y = $y+30;
	...
	public function createletter()
	{
		...
		return view("Offerletter/createletter",compact('departmentDetails','productDetails','designationDetails'));
	}
	...
	public function generateOfferLetterPost(Request $request)
	{
		...
		$modData->package_id = $selectedFilter['offerLetterData']['package_id'];
		$modData->monthly_package = $selectedFilter['offerLetterData']['monthly_package'];
		
		$modData->save();
		
		$request->session()->flash('message','Offer Letter generated Successfully.');
        return redirect('pdfOfferLetter/'.$modData->id);
	}
