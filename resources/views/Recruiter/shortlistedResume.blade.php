...
.col-lg-4.bottom-block-div {
 ...
 <form action="{{ url('shortlistedResumeFilter') }}" id="startS"method="post">
   ...
   <div class="col-md-3">
      <div class="form-group">
         <div class="input-group date" id="datetimepicker6">
            ...
            <input type="text" class="form-control" id="datef" name="date_from" value="{{$sessionValues['selecteddateFrom_shortlist']}}"/>
   ...
   <div class="col-md-3">
      <div class="form-group">
         <div class="input-group date" id="datetimepicker7" >
			 <label>Date To</label>
            <input type="text" class="form-control" id="datet" name="date_to" value="{{$sessionValues['selecteddateTo_shortlist']}}"/>
            <span class="input-group-addon">
         <i class="fa fa-calendar" aria-hidden="true"></i>
            </span>
         </div>
        ...
        <div class="form-group col-md-3">
            ...
            <select id="consultancy" class="selectpicker" name="consultancy">
				...
				@foreach($datas['consultancyLists'] as $_consultancy)
					<option value="{{ $_consultancy->id }}" @if($sessionValues['selectedConsultancy_shortlist'] == $_consultancy->id) selected="selected" @endif>{{ $_consultancy->consultancy_name }}</option>
				@endforeach
			</select>
		</div>
		<div class="form-group col-md-3">
			...
			@foreach($datas['status'] as $key=>$value)
					<option value="{{ $key }}" @if($sessionValues['selectedresumeStatus_shortlist'] == $key) selected="selected" @endif>{{ $value }}</option>
			@endforeach
...
<label>Records Per Page</label>
			...
			<option value="8" @if($sessionValues['selectedrecordperpage_shortlist'] == 8) selected="selected" @endif>8</option>
			<option value="16" @if($sessionValues['selectedrecordperpage_shortlist'] == 16) selected="selected" @endif>16</option>
			<option value="24" @if($sessionValues['selectedrecordperpage_shortlist'] == 24) selected="selected" @endif>24</option>
			<option value="64" @if($sessionValues['selectedrecordperpage_shortlist'] == 64) selected="selected" @endif>64</option>
	...
	<div class="text-right col-lg-9">
					...
					<a href="{{url('resetSearchShortlisted')}}"  title="View" class="btn btn-danger margin-right">Reset</a>
...
<div class="container-fluid panel panel-default">
  <div class="panel-heading">
         Shortlisted Resume <span>({{count($datas['resumeCount'])}} Resumes Found)</span>
  </div>
   <div class="panel-body bottom-block">
 <div class="pagination-container" >
  ...
  <ul class="pagination justify-content-center">
	...
	<?php
	$limitCount = $sessionValues['selectedrecordperpage_shortlist'];
...
$(document).ready(function(){
	 ...
	 $.ajax({
							type: "GET",
							url: "{{url('showResumeShortListed')}}/{{$limitCount}}/0",
...
function runpage(limit,skip)
{
	 ...
	 $.ajax({
							type: "GET",
							url: "{{url('showResumeShortListed')}}/"+limit+"/"+skip,
...
function slideShow()
{
	if(document.getElementById('datef').value == '' || document.getElementById('datet').value == '' || document.getElementById('consultancy').value == '' || document.getElementById('statusId').value == '') 
	{
		alert("Kindly select From date,To date, Consultancy and Status(Any)");
	}
	else
	{
		...
		window.location.href= "{{url('getResumeshortlisted')}}/"+consultancy+"/"+datef+"/"+datet+"/"+statusId;
	}
}
</script>
<style>
...
function changeStatus(rID,sID)
{
	document.getElementById("ResumeId").value = rID;
	$("#Cstatus").val(sID);
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
		  		  ...
		  		  <button type="button" class="btn btn-success" data-dismiss="modal" onclick="ChangeStatusOfResume();" >Done</button>
...
<script>
function ChangeStatusOfResume()
{
		if($("#comment").val() == '')
		{
			alert("kindly add some comments to change status.");
		}
		else
		{
		 var idtxt = 'id_'+$('#ResumeId').val();
		 $.ajax({
		  type: 'POST',
		  url: "{{url('setCandidateStatus')}}",
		  data:  { 'comment' : $("#comment").val(),'_token':'{{ csrf_token() }}','idtext':idtxt,'status':$("#Cstatus").val() },
		  dataType: "text",
...
<input type="hidden" id="ResumeId" value="0">
@stop